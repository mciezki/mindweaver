...
export const useUpdateProfileMutation = () => {
    ...
    return useMutation<UpdateProfileResponse, Error, Partial<ProfileUpdateSchema>>({
        mutationFn: async (data: Partial<ProfileUpdateSchema>) => {
            const formData = new FormData();

            for (const key in data) {
                if (data.hasOwnProperty(key) && (key !== 'profileImage' && key !== 'coverImage')) {
            ...
            const response = await patch<UpdateProfileResponse, FormData>(PRIVATE_API.update, formData, {
                headers: {
