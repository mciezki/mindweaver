import { PaginationOptions, PublicThreadsList, ThreadResponse } from '@mindweave/types';
...
export const getPublicThreadsList = async (options: PaginationOptions): Promise<PublicThreadsList> => {
    ...
    try {
        ...
        const totalCount = await prisma.socialThread.count({ where: whereCondition });
